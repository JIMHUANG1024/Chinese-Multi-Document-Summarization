...
class TransformerInterEncoder(nn.Module):
    def __init__(self, num_layers, d_model, heads, d_ff,
        ...
        self.transformer_layers = nn.ModuleList(
            [TransformerInterLayer(d_model, inter_heads, d_ff,
                                   dropout) if i in inter_layers else TransformerEncoderLayer(
    ...
    def forward(self, src):
        ...
        mask_local = ~src.data.eq(padding_idx).view(batch_size * n_blocks, n_tokens)  # 1 - src.data.eq(
        # padding_idx).view(batch_size * n_blocks, n_tokens)
        ...
        for i in range(self.num_layers):
            if (self.transformer_types[i] == 'local'):
                word_vec = self.transformer_layers[i](word_vec, word_vec,
                                                      ~mask_local)
                # self.transformer_layers[i](word_vec, word_vec, 1 - mask_local)  # all_sents * max_tokens * dim
            elif (self.transformer_types[i] == 'inter'):
                word_vec = self.transformer_layers[i](word_vec, ~mask_local, ~mask_block, batch_size,n_blocks)
                # self.transformer_layers[i](word_vec, 1 - mask_local, 1 - mask_block, batch_size,n_blocks)  #
        ...
        unpadded = torch.stack(
            [torch.cat([p, torch.zeros(max_l - p.size(0), src_features.size(-1)).to(self.device)]) for p in unpadded],
            1)
